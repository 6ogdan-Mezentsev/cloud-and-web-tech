...
1. Запускаем веб-серввер: 
```bash
brew services restart nginx
```
2. Запускаем сервер с нашим API:
```
node index.js
```
3. Теперь открываем страницу фронта по адресу `https://front.local/`:

<img width="1798" height="826" alt="image" src="https://github.com/user-attachments/assets/55b4364d-9287-4ab5-b82d-85818f0f2daa" />

4. И далее пробуем получить json от API(дотупный только по домену `api.local`), делая запрос по кнопке:

<img width="1486" height="774" alt="image" src="https://github.com/user-attachments/assets/e23c5afa-a735-4a1f-a5e9-b564bf206881" />

И видим полученный текст =)
Всё работает по https и браузер ни на что не ругается.

В результате данной лаборатоной работы я получил навыки настройки nginx с поддержкой HTTPS и самоподписанных SSL-сертификатов, организации виртуальных хостов для нескольких проектов, настройки CORS и раздачи статических файлов через alias. Также пригодилось умение анализировать логи веб-сервера для решения ошибок конфигураций.
